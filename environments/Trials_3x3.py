...
class TrialEnv(MiniGridEnv):
    ...
    def extractState(self):
        ...
        # if ((state[39] > 0) or (state[41] > 0)) and self.flag_cue_removal:
            # remove cues
            self.flag_cue_removal = False
            # print("removing cues")
            if self.random_cues:
        ...
        # variable view size 5x5
        # part_state = np.zeros(25)
        p_state = np.zeros(25)
        current_agent_pos = self.agent_pos[1] * self.grid.width + self.agent_pos[0]
        partial_view_size = self.agent_view_size + 2
        # p_state = np.zeros(49)
        # current_agent_pos = self.agent_pos[1] * self.grid.width + self.agent_pos[0]
        # partial_view_size = self.agent_view_size + 4

        # create a 30x30 pad state and copy over the original state
        pad_state = np.full((900,), -1)
        for i in range(9):
            idx_start = (i + 10) * 30 + 10  # 30 is grid width and 10 column offset
            idx_state = i * 9
            pad_state[idx_start : idx_start + 9] = state[idx_state : idx_state + 9]
        # print("state", state)
        # print("pad_state", pad_state)
        # print("current_agent_pos", current_agent_pos)
        # print("agent_dir", agent_dir)
        if agent_dir == 1:  # down
            start_row = self.agent_pos[1]
            end_row = self.agent_pos[1] + partial_view_size
            start_col = self.agent_pos[0] - (partial_view_size - 1) // 2
            end_col = self.agent_pos[0] + (partial_view_size - 1) // 2
        elif agent_dir == 3:  # up
            start_row = self.agent_pos[1] - partial_view_size + 1
            end_row = self.agent_pos[1] + 1
            start_col = self.agent_pos[0] - (partial_view_size - 1) // 2
            end_col = self.agent_pos[0] + (partial_view_size - 1) // 2
        elif agent_dir == 4:  # right
            start_row = self.agent_pos[1] - (partial_view_size - 1) // 2
            end_row = self.agent_pos[1] + (partial_view_size - 1) // 2
            start_col = self.agent_pos[0]
            end_col = self.agent_pos[0] + partial_view_size - 1
        elif agent_dir == 2:  # left
            start_row = self.agent_pos[1] - (partial_view_size - 1) // 2
            end_row = self.agent_pos[1] + (partial_view_size - 1) // 2
            start_col = self.agent_pos[0] - partial_view_size + 1
            end_col = self.agent_pos[0]

        # add offset of the pad of 10 rows and 10 columns
        start_row += 10
        end_row += 10
        start_col += 10
        end_col += 10
        ...
        if self.partial:
            for i in range(partial_view_size):
                idx = i * partial_view_size
                start_r = (start_row + i) * 30  # 30 is pad_state grid width
                p_state[idx : idx + partial_view_size] = pad_state[
                    start_r + start_col : start_r + end_col + 1
                ]
        ...
        # # make allo view from top (full view) and ego view (partial)
        # # pad part_state to make it 82 dim vector
        # if self.task_id in (6, 7):
        #     return torch.FloatTensor(part_state)
        #     # return torch.FloatTensor(state)
        # else:
        #     # ego with only 1 cell view
        #     ego_state = np.full(9, -1)
        #     # ego_state[4] = part_state[4]
        #     ego_state[3:5] = part_state[3:5]
        #     # # ego with partial view
        #     # part_state = np.pad(
        #     #     part_state,
        #     #     (0, len(state) - len(part_state)),
        #     #     "constant",
        #     #     constant_values=0,
        #     # )
        #     # # print("part_state", part_state)
        #     return torch.FloatTensor([ego_state])
